...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        #     # PositionalEmbedding(base_channels, mask_emb_scale),
        #     nn.Linear(512, mask_emb_dim),
        #     nn.SiLU(),
        #     nn.Linear(mask_emb_dim, mask_emb_dim),
        # ) if time_emb_dim is not None else None
    ...
    def forward(self, x, time=None, y=None, map=None, training=True):
        ip = self.initial_pad
        if ip != 0:
            x = F.pad(x, (ip,) * 4)
        ...
        # if training:
            # 256
            # map *= torch.FloatTensor(map.shape).uniform_(0.4, 1.0).cuda()
            # map[:,:, 37:37+24, 21:21+24] *= torch.FloatTensor(map[:,:, 37:37+24, 21:21+24].shape).uniform_(0.4, 1.0).cuda()
            # map[:,:, 29:29+24, 22:22+24] *= torch.FloatTensor(map[:,:, 29:29+24, 22:22+24].shape).uniform_(0.4, 1.0).cuda()
            # map[:,:, 23:23+16, 13:13+16] *= torch.FloatTensor(map[:,:, 23:23+16, 13:13+16].shape).uniform_(0.4, 1.0).cuda()
            # map[:,:, 23:23+16, 32:32+16] *= torch.FloatTensor(map[:,:, 23:23+16, 32:32+16].shape).uniform_(0.4, 1.0).cuda()

            #512
            # map *= torch.FloatTensor(map.shape).uniform_(0.6, 1.0).cuda()
            # map[:,:, 74:74+48, 42:42+48] *= torch.FloatTensor(map[:,:, 74:74+48, 42:42+48].shape).uniform_(0.6, 1.0).cuda()
            # map[:,:, 58:58+48, 44:44+48] *= torch.FloatTensor(map[:,:, 58:58+48, 44:44+48].shape).uniform_(0.6, 1.0).cuda()
            # map[:,:, 46:46+32, 26:26+32] *= torch.FloatTensor(map[:,:, 46:46+32, 26:26+32].shape).uniform_(0.6, 1.0).cuda()
            # map[:,:, 46:46+32, 64:64+32] *= torch.FloatTensor(map[:,:, 46:46+32, 64:64+32].shape).uniform_(0.6, 1.0).cuda()

        # map[:,:, 37:37+24, 21:21+24] *= 0.8
